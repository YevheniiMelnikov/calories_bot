...
async def on_startup(message: types.Message):
    await message.answer('Приветствую 🙂\nВыберите действие', reply_markup=on_startup_button)
...
async def calculate(callback: types.CallbackQuery):
    ...
    await callback.message.answer('Укажите ваш пол', reply_markup=gender_button)
    await callback.answer()
...
async def goal_input(message: types.Message, state: FSMContext):
    ...
    await BotState.next()
    await bot.send_message(message.from_user.id, 'Выберите вашу цель', reply_markup=goal_button)
...
async def gender_input(message: types.Message, state: FSMContext):
    ...
    await BotState.next()
    await bot.send_message(message.from_user.id, 'Укажите ваш вес (кг)')
...
async def weight_input(message: types.Message, state: FSMContext):
    ...
    await BotState.next()
    await bot.send_message(message.from_user.id, 'Укажите ваш рост (см)')
...
async def height_input(message: types.Message, state: FSMContext):
    ...
    await BotState.next()
    await bot.send_message(message.from_user.id, 'Укажите ваш возраст')
...
async def activity_input(message: types.Message, state: FSMContext):
    ...
    await state.finish()
    await bot.send_message(message.from_user.id, 'Повторить расчёт?', reply_markup=on_startup_button)
